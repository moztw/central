...
$faq[] = array("如何製作固定寬度的文字", "MozTW 特有的 BBCode 讓你在你的文字中使用固定寬度的樣式. 就如下方的例子: <ul><li>要製作一段固定寬度的文字，將它放在 <b>[tt][/tt]</b>, 例如：<br /><b>[tt]</b>monospaced text<b>[/tt]</b><br />就會變成 <tt>monospaced text</tt><br /><small>(from <a href=\"http://sillydog.org/forum/\">SillyDog701 Message Centre</a>)</small></li></ul>");
...
$faq[] = array("回覆時引用文字", "有兩種方式可讓您引用文章內容, 顯示引用來源及直接引用.<ul><li>當您在討論版面使用引言回覆時, 您會注意到文章內容已被加入回覆內容視窗內  <b>[quote=\"\"][/quote]</b> 的區段. 這個方法允許您引用某位發表者的文章內容並顯示來源! 例如要引用小竹子的文章內容時, 您必須輸入:<br /><br /><b>[quote=\"小竹子\"]</b>小竹子的文章內容放置在這裡<b>[/quote]</b><br /><br />這將會在顯示時, 自動加上: <b>小竹子 寫到:</b> 實際的內容. 請記得您<b>必須</b>在\"\" 裡指定引用者的名稱.<br /><br /></li><li>第二種方法允許您直接引用. 要使用這個標籤時, 您必須使用 <b>[quote][/quote]</b> 標籤. 而這種使用方式將會只會顯示簡單的引用功能, 例如: <b>引用回覆: </b>您所指定的文章內容.</li></ul>");
