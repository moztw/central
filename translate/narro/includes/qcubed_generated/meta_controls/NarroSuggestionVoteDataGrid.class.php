<?php
	...
	class NarroSuggestionVoteDataGrid extends NarroSuggestionVoteDataGridGen {
        ...
        public function colSuggestion_Render( NarroSuggestionVote $objVote ) {
            return
                str_replace(
            		'?l=' . QApplication::$TargetLanguage->LanguageCode,
                	'?l=' . $objVote->Suggestion->Language->LanguageCode,
                    NarroLink::Translate(0, '', NarroTranslatePanel::SHOW_ALL, "'" . $objVote->Suggestion->Text->TextValue . "'", 0, 0, 10, 0, 0, NarroString::HtmlEntities($objVote->Suggestion->SuggestionValue))
                );
        }
        
        public function colLanguage_Render( NarroSuggestionVote $objVote ) {
            return t($objVote->Suggestion->Language->LanguageName);
        }
        
        public function colCreated_Render( NarroSuggestionVote $objVote ) {
            $objDateSpan = new QDateTimeSpan(time() - $objVote->Created->Timestamp);
